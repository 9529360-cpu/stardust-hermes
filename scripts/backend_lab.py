...
def git(*args: str) -> str:
    proc = subprocess.run([str(GIT), "-C", str(REPO), *args], capture_output=True, text=True, encoding="utf-8",
...
def pid_alive(pid: int) -> bool:
    ...
    if os.name == "nt":
        ...
        return True
    try:
        os.kill(pid, 0)  # windows-footgun: ok — POSIX-only branch after explicit os.name == "nt" handling
        return True
    except OSError:
        return False
