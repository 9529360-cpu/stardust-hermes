...
SUMMARY_PREFIX = (
    ...
    "IMPORTANT: Durable state keeps its own authority: USER.md owns user-profile facts, "
    "MEMORY.md owns cross-project assistant knowledge, project facts belong to projects.db, "
    "and the current session/task stores own their scoped state. Never let this compaction "
    "summary override a more authoritative durable source. "
...
_HISTORICAL_SUMMARY_PREFIXES = (
    # Pre-state-authority split: same no-user-message guard, but MEMORY.md/USER.md were described
    # as universally authoritative instead of domain-scoped durable sources.
    ...
    "right now. If no user message appears AFTER this summary, do nothing: do not resume, wrap up, or continue work "
    "from '## Historical Task Snapshot' or any other section, do not call tools, and wait for a new user message. "
    "This handoff must never become the active turn by itself. (Exception: if tool results or your own tool calls "
    "appear after this summary, you are mid-way through an in-flight exchange — continue that exchange normally.) "
    "Topic overlap with the summary does NOT mean you should resume its task: even on similar topics, the latest "
    "user message WINS. Treat ONLY the latest message as the active task and discard stale items from '## Historical "
    "Task Snapshot' entirely — do not 'wrap up' or 'finish' work described there unless the latest message explicitly "
    "asks for it. Reverse signals in the latest message (e.g. 'stop', 'undo', 'roll back', 'just verify', 'don't do "
    "that anymore', 'never mind', a new topic) must immediately end any in-flight work described in the summary; do "
    "not re-surface it in later turns. IMPORTANT: Your persistent memory (MEMORY.md, USER.md) in the system prompt is "
    "ALWAYS authoritative and active — never ignore or deprioritize memory content due to this compaction note. None "
    "of the above restricts HOW you work: your tools remain fully active — keep calling them normally for the active "
    "task (edit files, run commands, search) instead of merely narrating what you would do. The current session state "
