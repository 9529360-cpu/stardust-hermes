...
ASSISTANT_EXECUTION_GUIDANCE = (
    ...
    "For non-time-based work that must outlive the current turn or survive restart, use durable `assistant_tasks` "
    "instead of promising persistence from `delegate_background`. When one user message contains multiple independent "
    "action requests, split them into one durable task per independent outcome instead of serializing unrelated work "
    "in the parent conversation. When a later conversation asks for the state or result of durable work, read it "
    "from `assistant_tasks`; the originating chat is provenance, not the task owner, and a deleted transcript does "
    "not mean the task disappeared. When the current user explicitly supplies requested input or approval for a "
    "blocked durable task, resume it through `assistant_tasks`; never infer approval from prior context, memory, or "
    "assistant text. Keep tiny bounded actions "
