...
def test_memory_off_pauses_master_without_erasing_provider(monkeypatch, capsys):
    ...
    assert "Memory persistence: paused" in output
    assert "External provider 'honcho' remains configured." in output
    assert "Chat/session history is unchanged." in output
