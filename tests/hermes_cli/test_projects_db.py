...
def test_connect_revalidates_cached_path_after_external_restore(tmp_path):
    """Replacing projects.db under a long-lived process must rerun migrations."""
    import sqlite3

    db_path = tmp_path / "projects.db"
    with pdb.connect_closing(db_path):
        pass
    restored = tmp_path / "restored-projects.db"
    raw = sqlite3.connect(str(restored))
    raw.executescript(pdb.SCHEMA_SQL)
    raw.execute("ALTER TABLE projects DROP COLUMN color")
    raw.close()
