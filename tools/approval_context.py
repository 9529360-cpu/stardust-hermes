...
# One already-authorized exact tool call may suppress the recoverable approval layer once. The caller
# id is generated by the model/runtime and bound again around the actual registry dispatch; this keeps
# the grant from leaking to a later command even if a tool fails before it reaches its approval check.
_exact_tool_approval_call_id: contextvars.ContextVar[str] = _ctx("exact_tool_approval_call_id")
...
def arm_exact_tool_call_approval(tool_call_id: str) -> None:
    """Arm one recoverable approval-layer bypass for exactly ``tool_call_id``.

    This is intentionally not a session/permanent allowlist. The durable personal-assistant approval
    bridge calls it only after consuming an exact args fingerprint from the task ledger. The terminal
    security floor (hardline/sudo/user deny) runs before ``_get_approval_mode`` and therefore remains
    authoritative even when this one-shot grant is consumed.
    """
    clean = str(tool_call_id or "").strip()
    if not clean:
        raise ValueError("exact tool approval requires tool_call_id")
    _exact_tool_approval_call_id.set(clean)


def _consume_exact_tool_call_approval() -> bool:
    """Consume the armed grant iff the currently executing tool call has the same id.

    A mismatched non-empty runtime id retires the stale grant immediately. An empty runtime id leaves
    it untouched because pre-tool policy runs before observability ids are rebound around execution.
    """
    expected = str(_exact_tool_approval_call_id.get() or "")
    if not expected:
        return False
    current = str(_approval_tool_call_id.get() or "")
    if not current:
        return False
    _exact_tool_approval_call_id.set("")
...
def _get_approval_mode() -> str:
    """Return 'manual', 'smart', or 'off'; hosted-room policy remains authoritative.

    A matching exact-tool grant returns ``off`` exactly once for the recoverable approval layer. This
    is deliberately consumed only after hosted-room policy resolution and only while the runtime has
    rebound the same ``tool_call_id`` around actual execution.
    """
    try:
        ...
        pass
    if _consume_exact_tool_call_approval():
        return "off"
