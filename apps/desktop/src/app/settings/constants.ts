...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  fileReadMaxChars: 'Maximum characters Stardust can read from one file request.',
  approvals: {
    mode: 'How Stardust handles commands that need explicit approval.',
  ...
  voice: {
    ...
    voiceChatMode:
      'chained: speech-to-text → Stardust → text-to-speech with the providers below. gpt-live: one full-duplex OpenAI voice model (gpt-live-1) listens and talks, and hands every real request to Stardust — any model you have selected answers with the full toolset. Needs an OpenAI API key; the voice layer bills $0.05 per minute.',
    gptLive: {
      voice: 'Voice for GPT-Live mode. Custom voice IDs are accepted.',
      instructions:
        'Extra sentences for the live voice persona (tone, pace, language). Stardust keeps its own system prompt.'
    }
  },
  tts: {
    xai: {
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Stardust updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
