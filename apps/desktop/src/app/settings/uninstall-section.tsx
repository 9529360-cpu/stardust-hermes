...
export function UninstallSection() {
  ...
  const options: ModeOption[] = [
    {
      mode: 'gui',
      title: u.guiOnlyTitle,
      description: u.guiOnlyDescription,
      consequence: u.guiOnlyConsequence,
      needsAgent: false
    },
    {
      mode: 'lite',
      title: u.liteTitle,
      description: u.liteDescription,
      consequence: u.liteConsequence,
      needsAgent: true
    },
    {
      mode: 'full',
      ...
      consequence: u.fullConsequence,
      // Full removes the local agent and local user data; hide it when this
      // desktop is only a thin client for a remote backend.
      needsAgent: true
    }
  ]
    ...
    <div className="mx-auto mt-8 w-full max-w-2xl">
      ...
      <div className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-3">
        {loading ? (
          ...
          <div>
            <p className="text-sm font-medium text-destructive">{u.confirmUninstall}</p>
            <p className="mt-1 text-xs text-muted-foreground">{u.confirmRemoves(pendingOption.consequence)}</p>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-3">
              <Button disabled={running} onClick={() => void handleConfirm()} size="sm" variant="destructive">
                ...
                {running ? u.uninstalling : u.confirmAction}
              </Button>
          ...
          <div className="flex flex-col gap-2">
            <p className="text-sm font-medium">{u.uninstallHermes}</p>
            <p className="text-xs text-muted-foreground">{u.chooseDescription}</p>
