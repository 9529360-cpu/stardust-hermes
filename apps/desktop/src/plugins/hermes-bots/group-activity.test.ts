...
describe('epoch scoping', () => {
  it('queues follow-ups without cancelling the active turn or losing its reply delta', async () => {
    ...
    release('first reply')
    await drain(() => !room.gateway.calls.some(call => call.prompt.includes('follow-up')))
    await drain(() => Boolean(room.chat.$groupChats.get().Busy?.running))
    const followUp = room.gateway.calls.find(call => call.prompt.includes('follow-up'))
    expect(followUp).toBeDefined()
    expect(followUp?.prompt).toMatch(/follow-up[\s\S]*first reply/)
    expect(followUp?.prompt).not.toContain('first ask')
