...
describe('terminal kinds beyond completed', () => {
  ...
  it('gave_up: plain-words body, raw payload error only in detail; crashed and timed_out fall back to the task id', async () => {
    ...
    expect(gaveUp.message).toBe('Stardust couldn’t finish this task. Open Kanban to see why and reassign it.')
  ...
  it('gave_up without a payload error still gets the plain-words body and no empty detail noise', async () => {
    ...
    expect(lastNotify()).toMatchObject({
      kind: 'error',
      message: 'Stardust couldn’t finish this task. Open Kanban to see why and reassign it.',
      detail: 't101'
    })
  })

  it('retrying kinds (crashed/timed_out) say Stardust will retry and never expose worker/gateway vocabulary', async () => {
    const m = await loadModule()
    m.bindCompletionNotify(makeRest(() => 100) as never)
    ...
    for (const call of hostMock.notify.mock.calls) {
      ...
      expect(toast.title).toMatch(/Stardust will retry it automatically/)
...
describe('i18n routing', () => {
  ...
  it('falls back to the English bundle when the translator echoes the key', async () => {
    ...
    expect(lastNotify().title).toBe('Task took too long — Stardust will retry it automatically')
  })
})
