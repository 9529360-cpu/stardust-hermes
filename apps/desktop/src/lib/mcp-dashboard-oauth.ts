...
const UPDATE_BACKEND = 'Update the Stardust backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update Stardust Desktop to support MCP OAuth callbacks.')
  }
