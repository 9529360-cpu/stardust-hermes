...
export function BootFailureOverlay() {
  ...
  let hint: string
  // The electron boot path can flag a legacy managed-cloud backend-down (502/503/504) with
  ...
  } else if (cloudDown || remoteFailure) {
    // Legacy managed-cloud connections are treated exactly like any other
    // unreachable remote gateway. Stardust no longer sends users to a
    // first-party account/portal; recovery stays inside gateway settings,
    // retry, or the bundled local backend.
