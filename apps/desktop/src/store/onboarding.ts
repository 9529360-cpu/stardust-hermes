...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Stardust is ready', message: `${provider} connected.` })
}
...
async function completeWithModelConfirm(
  ...
  if (defaults) {
    ...
    try {
      ...
      onFail(error instanceof Error ? error.message : 'Stardust could not save the selected model.')
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ...
    : 'Connected, but Stardust still cannot resolve a usable provider.'
}
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'Stardust Desktop could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
