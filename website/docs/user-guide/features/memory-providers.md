...
Hermes Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be selected at a time. The master `memory.enabled` switch controls durable memory as a whole; the two built-in files can still be enabled or disabled independently beneath it.

## Quick Start

```bash
hermes memory setup      # interactive picker + configuration
hermes memory status     # show privacy + built-in + provider state
hermes memory off        # pause all durable memory; keep provider config
hermes memory on         # resume durable memory with the same config
```
...
When `memory.enabled: true` and an external provider is selected, Hermes automatically:
...
While the master switch is on, the external provider is additive to whichever built-in targets are enabled. Set `memory.enabled: false` (or run `hermes memory off`) to stop built-in persistence plus provider initialization, sync, prefetch, hooks, and memory-tool I/O without clearing the provider selection or credentials. Chat/session history is a separate subsystem and remains available. Re-enabling memory does not backfill turns that occurred while it was off.
