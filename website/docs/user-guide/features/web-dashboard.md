...
- **Memory** — pick the external memory provider (or built-in only), and reset the built-in `MEMORY.md` / `USER.md` stores. A reset fences stale in-flight/staged built-in writes and active chats reload the reset files on their next turn; external-provider data is not deleted.
...
| `POST /api/memory/reset` | Reset built-in memory. Body: `{target: all\|memory\|user}`; response includes `active_session_behavior: "refresh_on_next_turn"` on generation-aware backends |
