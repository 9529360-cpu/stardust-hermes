...
- **Memory** — pick the external memory provider (or built-in only), and reset the built-in `MEMORY.md` / `USER.md` stores. Reset fences stale in-flight/staged built-in writes; active chats reload the reset files on their next turn. External-provider data is not deleted.
...
| `POST /api/memory/reset` | Reset built-in memory. Body: `{target: all\|memory\|user}`; generation-aware backends return `active_session_behavior: "refresh_on_next_turn"` |
