...
def main(catalog_dir: Path = DEFAULT_CATALOG_DIR, output_dir: Path = DEFAULT_OUTPUT_DIR,
    ...
    with open(output_dir / "plugin-catalog.json", "w", encoding="utf-8") as f:
        # Runtime authority artifact: deterministic so the reviewed committed copy can be
        # regenerated byte-for-byte in CI. Page freshness lives in plugins-meta.json instead.
        json.dump({"entries": load_raw_entries(catalog_dir), "removed": load_removed(catalog_dir)},
