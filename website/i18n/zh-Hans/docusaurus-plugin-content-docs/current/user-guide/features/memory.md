...
两个文件均存储于 `~/.hermes/memories/`。当前回合使用冻结快照；如果任一文件在磁盘上发生变化，Stardust 会在下一回合边界检测到漂移、重新加载内置记忆，并且只有在渲染后的记忆内容确实变化时才重建缓存的系统 prompt。Agent 通过 `memory` 工具管理自身记忆——可添加、替换或删除条目。
...
每个会话第一次构建 prompt 时，记忆条目从磁盘加载并以冻结块的形式渲染到系统 prompt 中：
...
**回合冻结快照模式：** 正在执行的模型回合不会被中途修改。写入会立即持久化到磁盘；在下一回合边界，Stardust 会检查文件身份、内容摘要和 reset generation，必要时重新加载，并且只有渲染后的记忆快照发生变化时才使 prompt 缓存失效。这样既保持当前回合稳定，又能让正常的记忆修改从下一回合开始生效。

## 重置 / 忘记语义

Command Center 中的 **重置记忆**、**重置画像** 和 **全部重置** 只影响内置的 `MEMORY.md` / `USER.md` 文件，不会删除已配置外部记忆提供商中的数据。

每次重置都会在与内置记忆写入相同的文件锁下推进该目标的持久 reset generation，从而形成真正的忘记边界：

- 已经开始的回合仍可能携带它启动时的旧 prompt 内容，但旧 `MemoryStore` 不能再把该目标写回磁盘；
- 重置前已暂存、重置后才批准的记忆写入会被拒绝，不会把已忘记的事实恢复回来；
- 下一回合会重新加载已重置目标，并从 prompt 中移除已删除的内置记忆；
- 文件与 generation marker 都位于该 profile 自己的 `HERMES_HOME` 下，因此重置保持 profile 隔离。

如果 Desktop 连接的是不返回回合刷新契约的旧后端，Desktop 会明确提示新建对话，而不是假装旧会话已经刷新。
...
没有 `read` 操作——内置记忆会自动注入系统 prompt；当文件发生变化时，会在回合边界刷新。Agent 将其记忆作为对话上下文的一部分来查看。
